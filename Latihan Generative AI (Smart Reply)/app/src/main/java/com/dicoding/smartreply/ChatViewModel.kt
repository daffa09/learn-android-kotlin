...
class ChatViewModel : ViewModel() {
    ...
    private val _smartReplpyOptions = MediatorLiveData<List<SmartReplySuggestion>>()
    val smartReplyOptions: LiveData<List<SmartReplySuggestion>> = _smartReplpyOptions
    ...
    private fun clearSmartReplyOptions() {
        _smartReplpyOptions.value = ArrayList()
    }

    private fun initSmartReplyOptionsGenerator() {
        _smartReplpyOptions.addSource(pretendingAsAnotherUser) {isPretendingAsAnotherUser ->
            ...
            if (list.isNullOrEmpty()) {
                ...
                generateSmartReplyOptions(list, isPretendingAsAnotherUser)
                    .addOnSuccessListener { result ->
                        _smartReplpyOptions.value = result
                    }
            }
        }

        _smartReplpyOptions.addSource(chatHistory){conversation ->
            val isPretendingAsAnotherUser = pretendingAsAnotherUser.value

            if (isPretendingAsAnotherUser != null && conversation.isNullOrEmpty()) {
                return@addSource
            } else {
                generateSmartReplyOptions(conversation, isPretendingAsAnotherUser!!)
                    .addOnSuccessListener { result ->
                        _smartReplpyOptions.value = result
                    }
            }
            ...
            .continueWith{ task ->
                ...
                when(result.status) {
                    SmartReplySuggestionResult.STATUS_NOT_SUPPORTED_LANGUAGE ->
                        _errorMessage.value = "Unable to generate options due to a non-english languange was used"
